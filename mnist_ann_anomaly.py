...
def load_data(batch_size=128):
    ...
    val_loader_reduced = DataLoader(TensorDataset(val_data_reduced, val_labels_reduced), batch_size=batch_size, shuffle=False) # shuffle is optional
...
def load_ann_models():
    """
    Load all ANN models from the models/mnist/ann directory.
    ...
    print(f"\nSuccessfully loaded {len(models)} ANN models")
...
def compute_reconstruction_errors(models, model_names, val_loader, device):
    ...
    Following the methodology from mnist.ipynb.
    ...
    """
    criterion = nn.MSELoss(reduction='none')  # Get per-sample losses
    ...
    for idx, (model, model_name) in enumerate(zip(models, model_names)):
        ...
        model.eval()
        with torch.no_grad():
            for images_batch, labels_batch in tqdm(val_loader, desc=f"Model {idx+1}"):
                # Move data to device
                images_batch = images_batch.to(device)
                
                # Get reconstructions from ANN
                reconstructions = model(images_batch)
                
                # Calculate per-sample reconstruction errors (MSE)
                errors = criterion(reconstructions, images_batch).mean(dim=1)  # Mean over features
                
                # Store predictions and labels
                all_predictions[idx].extend(errors.cpu())
                all_labels[idx].extend(labels_batch)
    
    # Convert to tensors
    for idx in range(len(all_predictions)):
        all_predictions[idx] = torch.stack(all_predictions[idx])
        all_labels[idx] = torch.tensor(all_labels[idx])
...
def analyze_anomaly_detection(all_predictions, all_labels, model_names):
    """
    Analyze anomaly detection performance following mnist.ipynb methodology.
    ...
    for idx, model_name in enumerate(model_names):
        ...
        sorted_labels[idx] = labels[sorted_indices]
        
        # Binary labels: 0 for digit '0' (anomalous), 1 for digits '1'-'9' (normal)
        binary_labels = (sorted_labels[idx] != 0).float()
        ...
        for threshold in thresholds:
            # Calculate threshold value as percentile
            threshold_value = torch.quantile(sorted_predictions, threshold)
            
            # Predictions: 1 if error > threshold (anomalous), 0 if error <= threshold (normal)
            binary_predictions = (sorted_predictions > threshold_value).float()
            
            # Calculate confusion matrix values
            tp = torch.sum((binary_predictions == 1) & (binary_labels == 0))  # True positives
            fp = torch.sum((binary_predictions == 1) & (binary_labels == 1))  # False positives
            tn = torch.sum((binary_predictions == 0) & (binary_labels == 1))  # True negatives
            fn = torch.sum((binary_predictions == 0) & (binary_labels == 0))  # False negatives
        ...
        results[model_names[idx]] = {
            ...
            'all_fprs': fprs[idx],
            'predictions': all_predictions[idx],
            'labels': sorted_labels[idx]
        }
    
    return results

def plot_error_distributions(results, model_names, save_dir="results/ann_anomaly_plots"):
    """
    ...
    for arch in architectures:
        ...
        fig.suptitle(f'Error Distributions - {arch}', fontsize=16, fontweight='bold')
...
def plot_roc_curves(results, model_names, save_dir="results/ann_anomaly_plots"):
    """
    ...
    fig, axes = plt.subplots(1, 3, figsize=(18, 5))
    fig.suptitle('ROC Curves - ANN Anomaly Detection', fontsize=16, fontweight='bold')
    ...
    for i, arch in enumerate(architectures):
        ...
        for latent_size in latent_sizes:
            ...
            if model_name in results:
                ...
                binary_labels = (model_results['labels'] != 0).int()  # 1 for normal (digits 1-9), 0 for anomalous (digit 0)
                auc_score = roc_auc_score(binary_labels, model_results['predictions'])
...
def plot_performance_summary(results, model_names, save_dir="results/ann_anomaly_plots"):
    """
    ...
    fig.suptitle('ANN Anomaly Detection Performance Summary', fontsize=16, fontweight='bold')
    ...
    ax4.set_ylabel('Optimal Threshold (Percentile)')
...
def main():
    ...
    print("="*60)
    print("ANN ANOMALY DETECTION ANALYSIS")
    print("="*60)
    
    # Load ANN models
    models, model_names = load_ann_models()
    ...
    _, val_loader, _ = load_data(batch_size=128)
    ...
    save_dir = "results/ann_anomaly_plots"
    os.makedirs(save_dir, exist_ok=True)
    ...
    # Save results
    results_path = "results/ann_anomaly_results.pt"
